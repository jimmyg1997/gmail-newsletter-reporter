...
# -*- coding: utf-8 -*-
"""
    [*] Description : Gmail Newsletter Reporter
...
class Controller:
    ...
    def run_initialization(self, enable_telegram: bool):
        ...
        self.google_sheets_api = GoogleSheetsAPI(
            mk1=self.mk1, google_api=self.google_api
        )
        self.google_email_api = GoogleEmailAPI(mk1=self.mk1, google_api=self.google_api)
        self.google_drive_api = GoogleDriveAPI(mk1=self.mk1, google_api=self.google_api)
        self.google_docs_api = GoogleDocsAPI(mk1=self.mk1, google_api=self.google_api)
        ...
        self.data_loader = DataLoader(
            mk1=self.mk1, google_sheets_api=self.google_sheets_api
        )
    ...
    def run_get_email_report(self):
        ...
        sheets_reporter_tab_config_newsletters = str(
            self.mk1.config.get("google_sheets", "reporter_tab_config_newsletters")
        )
        sheets_reporter_tab_config_docs = str(
            self.mk1.config.get("google_sheets", "reporter_tab_config_docs")
        )
        sheets_reporter_tab_config_docs_ids = str(
            self.mk1.config.get("google_sheets", "reporter_tab_config_docs_ids")
        )
        sheets_reporter_tab_config_telegram = str(
            self.mk1.config.get("google_sheets", "reporter_tab_config_telegram")
        )
        sheets_reporter_tab_logs = str(
            self.mk1.config.get("google_sheets", "reporter_tab_logs")
        )

        # App storage
        fn_path_summary_txt = str(
            self.mk1.config.get("app_storage", "fn_path_summary_txt")
        )
        fn_path_summary_json = str(
            self.mk1.config.get("app_storage", "fn_path_summary_json")
        )

        # Attributes
        hours = self.args.hours
        days = self.args.days_diff
        today = dt.datetime.now() + dt.timedelta(days=days)
        ...
        logs = {
            ...
            "start_datetime": (today - dt.timedelta(hours=hours)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
        ...
        newsletters, newsletters_categories = (
            self.reporter.get_config_enabled_newsletters(
        ...
        emails[["label_ids", "date", "subject", "from", "body", "category"]] = (
            emails.progress_apply(
                lambda x: self.email_analyzer.expand_with_body_info(
                    x, newsletters_categories
                ),
                axis=1,
            )
        )
        self.mk1.logging.logger.info(
            f"(Controller.run_get_emails) [# emails] [Initially] = {len(emails)}"
        )
        ...
        self.mk1.logging.logger.info(
            f"(Controller.run_get_emails) [# emails] [Keeping only the newsletters interested] = {len(emails)}"
        )
        ...
        emails[
            ...
            lambda x: self.email_analyzer.get_email_summary_and_statistics(x), axis=1
        )
        ...
        logs["num_tokens_clean_summarized"] = emails[
            "num_tokens_clean_summarized"
        ].sum() / len(emails)
        ...
        self.telegram_api.set_data(
            summary_per_category=summary_per_category, today=today, chat_ids=chat_ids
        )
        ...
        emails["archive_status"] = emails.progress_apply(
            lambda x: self.email_analyzer.archive_emails_after_summarizing(x), axis=1
        )
    ...
    def run(self):
        ...
        self.mk1.logging.logger.info("(Controller.run) All services initilalized")
        ...
        while True:
            ...
            self.mk1.logging.logger.info(
                f"(Controller.run) [Datetime = {today}, Schedule Time = {schedule_hour}] All services initilalized (except Telegram)"
            )

            if schedule_hour == today.hour:
                self.mk1.logging.logger.info(
                    f"(Controller.run) [Datetime = {today}, Schedule Time = {schedule_hour}] Report being generated ... "
                )
