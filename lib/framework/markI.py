...
class DataSet(object):
    ...
    def auto_update(self, db_dict: Dict[str, str]) -> None:
    ...
    def db_connect(self):
        ...
        if db_info["name"] is not None:
            ...
            db_obj = dataset.connect(
                os.path.join("sqlite:///", db_info["path"], db_info["name"])
            )
    ...
    def db_create_table(
        self, table_name: str = "", pk_name: str = "", pk_str: str = ""
    ):
        ...
        try:
            ...
            self.db.create_table(
                table_name, primary_id=pk_name, primary_type=self.get_pk_type(pk_str)
            )
            self.db.commit()
        except:
    ...
    def get_unique(self, table_name: str = "", col_name: str = ""):
        ...
        """
        return [
            list(each.values())[0] for each in self.db[table_name].distinct(col_name)
        ]
...
class Logger(object):
    ...
    def start_logger(self):
        ...
        logger.info(
            f"-------------------------------------- {self.app_oname} ({self.app_name}) Started .."
        )
        return logger
...
class M3(object):
    ...
    def __init__(self, config_obj):
        ...
        self.off_token = (
            self.get_off_token()
        )  # https://ugrafana.uberinternal.com/oidauth/offline
    ...
    def get_now_secs(self) -> Dict[str, Any]:
        ...
        """
        if self.push_client.emit_timing_in_ns:
            now_s = self.ns_to_s(self.push_client.current_nano_time())
        else:
            now_s = self.ms_to_s(self.push_client.current_micro_time())

        return {"epochs": now_s, "timestamp": self.s_to_datetime(now_s)}
    ...
    def convert_to_secs(self, x: int, x_type="") -> int:
        ...
        if x_type == "ns":
            return int(round(ns / 1000000000.0, 0))
        elif x_type == "ms":
            return int(round(ms / 1000000.0, 0))
    ...
    def push_gauge(self, key_: str, value_: int, tags_: Dict[str, str] = {}):
        ...
        ----
            ...
            :param: `key_`   (:obj: Dict[`str`,`str`]) - The additional tags dictionary
        """
        self.push_client.gauge(key=key_, value=value_, tags=tags_)
        return None
    ...
    def push_timing(self, key_: str, duration_: int, tags_: Dict[str, str] = {}):
        ...
        ----
            ...
            :param: `key_`      (:obj: Dict[`str`,`str`]) - The additional tags dictionary
        """
        self.push_client.gauge(key=key_, duration=duration_, tags=tags_)
        return None
    ...
    def query(
        self, query_str: str, from_: int, until_: int, tries: int = 3
    ) -> Dict[str, int]:
        ...
        results = self.read_client.fetch_query(
            query=query_str, from_timestamp=from_, until_timestamp=until_, retries=tries
        )
