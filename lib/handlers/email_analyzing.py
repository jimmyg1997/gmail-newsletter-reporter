...
# -*- coding: utf-8 -*-
"""
    [*] Description : Py3 class to analyze emails retrieved from Google Email API 
...
class EmailAnalyzer:
    ...
    def __init__(self, mk1, google_email_api=None, openai_api=None):
        """
    ...
    def expand_with_body_info(
        self, row: Dict[str, Any], newsletters_categories: Dict[str, str]
    ) -> pd.Series:
        """
    ...
    def get_email_summary_and_statistics(self, row: Dict[str, Any]) -> pd.Series:
        """
        ...
        """
        try:
            num_tokens_raw = len(row["body"].split(" "))

            body_clean = self.extract_content(html_text=row["body"])

            body_clean = self.remove_urls_and_patterns(text=body_clean)

            num_tokens_clean = len(body_clean.split(" "))

            body_clean_summarized = self.openai_api.generate_summary(text=body_clean)
            time.sleep(2)
