...
# -*- coding: utf-8 -*-
"""
    [*] Description : Py3 class to handle all data I/O operations
...
# -*-*-*-*-*-*-*-*-*-*-* #
class DataLoader:
    def __init__(self, mk1, google_sheets_api=None):
        ## System Design
        self.mk1 = mk1
    ...
    #         df = self.kirby_api.cast_to_hive_friendly_format(
    #             df=df, casting_map=casting_map
    #         )
    #         job_id = self.kirby_api.upload_to_hive(
    #             table_name=table_name, df=df, file_path=file_path
    #         )
    #         self.kirby_api.verify_job(
    #             job_id=job_id, table_name=table_name, time_sleep=time_sleep
    #         )
    #         self.mk1.logging.logger.info(
    #             f"(DataLoader.write_data_to_kirby) Data uploaded to Kirby table {table_name}"
    #         )

    #     except Exception as e:
    #         self.mk1.logging.logger.error(
    #             f"(DataLoader.write_data_to_kirby) Data uploading to kirby table {table_name} failed: {e}"
    #         )
    #         raise e

    ## *-*-*-*-*-*-*-*-*-*-*- QUERYRUNNER  -*-*-*-*-*-*-*-*-*-*- ##
    # @retry((queryrunner_client.lib.exceptions.ResultError),tries = 3, delay = 2)
    # def load_data_from_query_atlantis(
    #     self, report_id: str = "", params: Dict[str, Any] = {}, datacenter: str = "phx2"
    # ):
    #     try:
    #         data, _ = self.query_atlantis_api.run_query(
    #             report_id=report_id, datacenter=datacenter, parameters=params
    #         )
    #         self.mk1.logging.logger.info(
    #             "(DataLoader.load_data_from_query) Data Loaded"
    #         )
    #         return data

    #     except Exception as e:
    #         self.mk1.logging.logger.error(
    #             f"(DataLoader.load_data_from_query) Data loading failed: {e}"
    #         )
    #         raise e

    # @retry((queryrunner_client.lib.exceptions.ResultError),tries = 3, delay = 2)
    # def load_data_from_query_neutrino(
    #     self, report_id: str = "", params: Dict[str, Any] = {}, datacenter: str = "phx2"
    # ):

    #     try:
    #         data, _ = self.query_neutrino_api.run_query(
    #             report_id=report_id, datacenter=datacenter, parameters=params
    #         )
    #         # data = self.query_nr.query_data(report_id = report_id, params = params)[0]
    #         self.mk1.logging.logger.info(
    #             "(DataLoader.load_data_from_query_neutrino) Data Loaded"
    #         )
    #         return data

    #     except Exception as e:
    #         self.mk1.logging.logger.error(
    #             f"(DataLoader.load_data_from_query_neutrino) Data loading failed: {e}"
    #         )
    #         raise e
    ...
    def create_new_tab_to_google_sheets(
        self, spreadsheet_id: str, spreadsheet_tab_name: str
    ) -> None:
        ...
        """
        try:
            self.google_sheets_api.create_spreadsheet_tab(
                spreadsheet_id=spreadsheet_id, spreadsheet_tab_name=spreadsheet_tab_name
            )
            self.mk1.logging.logger.info(
                "(DataHandler.create_new_tab_to_google_sheets) New Tab created sucessfully"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.create_new_tab_to_google_sheets) New tab creation failed: {e}"
            )
            raise e

    @retry((ValueError, TypeError, KeyError), tries=3, delay=2)
    def get_google_sheets_tab_num_rows(
        self, spreadsheet_id: str, spreadsheet_range_name: str
    ) -> int:
        ...
        """
        try:
            num_rows = self.google_sheets_api.get_tab_num_dimension(
                spreadsheet_id, spreadsheet_range_name
            )
            self.mk1.logging.logger.info(
                f"(DataHandler.get_google_sheets_tab_num_rows) Tab {spreadsheet_range_name} has {num_rows} rows "
            )
            return num_rows

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.get_google_sheets_tab_num_rows) Tab number of rows retrieval failed : {e}"
            )
            raise e
    ...
    def clear_google_sheets_tab(
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                "(DataHandler.clear_google_sheets_tab) Tab was cleared sucessfully"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.create_new_tab_to_google_sheets) Tab clearing failed: {e}"
            )
            raise e
    ...
    def write_data_to_google_sheets(
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                "(DataHandler.write_data_to_google_sheets) Data Uploaded"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.write_data_to_google_sheets) Data uploading to google sheets failed: {e}"
            )
    ...
    def append_data_to_google_sheets(
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                "(DataHandler.append_data_to_google_sheets) Data Appended "
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.append_data_to_google_sheets) Data appending to google sheets failed: {e}"
            )
            raise e
    ...
    def load_data_from_google_sheets_tab(
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                f"(DataHandler.get_number_of_google_sheets_tab) Tab exists.{data.shape[0]} rows {data.shape[1]} columns retrieved successfully"
            )
            return data

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.get_number_of_google_sheets_tab) Tab or spreadsheet does not exist. Rows were not retrieval failed: {e}"
            )
            raise e
    ...
    def get_tab_url_from_google_sheets(
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                "(DataHandler.get_tab_url_from_google_sheets) Tab URL is retrieved successfully"
            )
            return url

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(DataHandler.get_tab_url_from_google_sheets)Tab URL retrieval failed : {e}"
            )
            raise e
