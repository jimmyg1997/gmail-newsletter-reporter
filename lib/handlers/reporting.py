...
# -*- coding: utf-8 -*-
"""
    [*] Description : Py3 class to generate the newsletter report for local (.txt) and cloud (Google Doc) storage purposes
...
class Reporter:
    def __init__(
        self, mk1, google_sheets_api=None, google_docs_api=None, google_drive_api=None
    ):
        self.mk1 = mk1
    ...
    def get_config(
        ...
        try:
            ...
            config = [config_newsletters, config_docs, config_docs_ids]

            self.mk1.logging.logger.info(
                f"(Reporter.get_config) All config dataframes (newsletters, docs, docs_ids) were retrieved succesfully"
            )
            return config

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter.get_config) Retrieving all config dataframes (newsletters, docs, docs_ids) failed: {e}"
            )
            raise e
    ...
    def write_summary_to_file(
        ...
        try:
                ...
                for category in newsletters.keys():
                    ...
                    if not emails_category.empty:
                        ...
                        for idx, email in emails_category.iterrows():
                            print(
                                f"[{category}] [{idx + 1}/{len(emails_category)}] Processing ..."
                            )
            ...
            with open(fn_path_summary_json, "w") as file:
                json.dump(
                    summary_per_category, file, indent=4
                )  # indent for pretty printing

            self.mk1.logging.logger.info(
                "(Reporter.write_summary_to_file) Appending summaries to file was successful"
            )
            return summary_per_category

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter.write_summary_to_file) Failed to append summaries: {e}"
            )
            raise e

    # ___________________________ (config) Newsletters ____________________________________ #
    def _parse_categories_dataframe_into_dict(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _generate_categories_mapping(self, newsletters: Dict):
        new_dict = dict()
    ...
    def get_config_enabled_newsletters(
        self, config_newsletters: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            self.mk1.logging.logger.info(
                f"(Reporter.get_config_enabled_newsletters) Newsletters Config was retrieved succesfully"
            )
            return newsletters, newsletters_categories

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter.get_config_enabled_newsletters) Newsletters Config retrieval failed: {e}"
            )
            raise e
        ...
        try:
            ...
            if doc_id in self.__null_values:  # Entering new month
                ...
                result = self.google_drive_api.copy_file(
                    file_id=docs_id_template, parent_folder=folder_id
                )
                ...
                self.google_drive_api.change_file_name(
                    file_id=doc_id,
                    new_name=doc_name.replace("Copy of ", "")
            ...
            self.mk1.logging.logger.info(
                f"(Reporter.get_doc_reporter_id) ID was retrieved successfully. Docs ID = {doc_id} "
            )
            return doc_id

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter.get_doc_reporter_id) Google Docs ID retrieval failed failed: {e}"
            )
            raise e

    def _append_header_to_doc_reporter(
        self, today: dt.datetime, doc_reporter_id: str
    ) -> None:
        try:
            ...
            self.mk1.logging.logger.info(
                f"(Reporter._append_header_to_doc_reporter) Appending the header {header} to doc ID : {doc_reporter_id} was successful"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter._append_header_to_doc_reporter) Appending the header to doc ID : {doc_reporter_id} failed: {e}"
            )
            raise e

    def _append_summary_to_doc_reporter(
        self, summary_per_category: Dict[str, str], doc_reporter_id: str
    ) -> None:
        try:
            for category, summary in summary_per_category.items():
                start_index = (
                    self.google_docs_api.get_document(document_id=doc_reporter_id)[
                        "content"
                    ][-1]["endIndex"]
                    - 1
                )
                ...
                if len(summary) > 0:
                    for summary_part in summary:
                        start_index = (
                            self.google_docs_api.get_document(
                                document_id=doc_reporter_id
                            )["content"][-1]["endIndex"]
                            - 1
                        )
            ...
            self.mk1.logging.logger.info(
                f"(Reporter._append_summary_to_doc_reporter) Appending text to doc ID : {doc_reporter_id} was successful"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter._append_summary_to_doc_reporter) Appending text to doc ID : {doc_reporter_id} failed: {e}"
            )
            raise e

    def _format_as_plain_table(self, df: pd.DataFrame) -> str:
        ...
        for index, row in df.iterrows():
            row_str = " | ".join(
                f"{str(index).ljust(col_widths[0])} | {str(row['Value']).ljust(col_widths[1])}"
            )
    ...
    def _format_df_for_google_docs(
        ...
        data_df = pd.DataFrame(data_dict, index=["Value"]).T

        if method == "plain_table":
            content = self._format_as_plain_table(df=data_df)

        elif method == "tabulate":
            content = tabulate(tabular_data=data_df, headers="keys", tablefmt=tablefmt)

        elif method == "json":
            content = json.dumps(obj=data_dict, indent=4, sort_keys=True)
    ...
    def _append_logs_to_doc_reporter(
        ...
        try:
            text_content = self._format_df_for_google_docs(
                data_dict=logs, method=method, tablefmt=tablefmt
            )
            ...
            self.mk1.logging.logger.info(
                f"(Reporter._append_logs_to_doc_reporter) Appending the logs to doc ID : {doc_reporter_id} was successful"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter._append_logs_to_doc_reporter) Appending the logs to doc ID : {doc_reporter_id} failed: {e}"
            )
            raise e
    ...
    def append_summary_to_doc_reporter(
        ...
        try:
            self._append_header_to_doc_reporter(
                today=today, doc_reporter_id=doc_reporter_id
            )
            ...
            self.mk1.logging.logger.info(
                f"(Reporter.append_text_to_doc) Appending text to doc ID : {doc_reporter_id} was successful"
            )

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(Reporter.append_text_to_doc) Appending text to doc ID : {doc_reporter_id} failed: {e}"
            )
            raise e
