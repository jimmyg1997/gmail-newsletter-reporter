...
# -*- coding: utf-8 -*-
"""
    [*] Description : Py3 wrapper for Telegram API
...
class TelegramAPI:
    def __init__(
        self, mk1: MkI, enable_polling: bool, google_sheets_api: GoogleSheetsAPI
    ):
        ## System Design
        self.mk1 = mk1
    ...
    def build_client(self):
        try:
            ...
            service = telegram.Bot(token=self.token_key)

            self.mk1.logging.logger.info(
                "(TelegramAPI.build_client) Service build succeeded"
            )
            return service

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(TelegramAPI.build_client) Service build failed: {e}"
            )
            raise e
            return None
    ...
    def build_updater(self):
        try:
            # Creating the OpenAI API client
            updater = Updater(self.token_key, use_context=True)

            self.mk1.logging.logger.info(
                "(TelegramAPI.build_updater) Updated build succeeded"
            )
            return updater

        except Exception as e:
            self.mk1.logging.logger.error(
                f"(TelegramAPI.build_updater) Updated build failed: {e}"
            )
            raise e
    ...
    def start_command(
        ...
        update.message.reply_text(
            "Hi! I will send you the daily Gmail news report every morning."
        )
    ...
    def register_chat_id_command(
        ...
        if not chat_id in chat_ids["chat_ids"].values:
            ...
            update.message.reply_text(
                f"Your chat ID is {chat_id} and has been registered for the Daily Newsletter Reporter\n𝐍𝐨. 𝐨𝐟 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝 𝐮𝐬𝐞𝐫𝐬 = {chat_ids.shape[0] + 1}"
            )
        else:
            update.message.reply_text(
                f"Your chat ID is {chat_id} is already registered!\n𝐍𝐨. 𝐨𝐟 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝 𝐮𝐬𝐞𝐫𝐬 = {chat_ids.shape[0]}"
            )
    ...
    def unregister_chat_id_command(
        ...
        if chat_id in chat_ids["chat_ids"].values:
            ...
            update.message.reply_text(
                f"Your chat ID {chat_id} has been unregistered from the Daily Newsletter Reporter. \n 𝐍𝐨. 𝐨𝐟 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝 𝐮𝐬𝐞𝐫𝐬 = {updated_chat_ids.shape[0]}"
            )
        else:
            update.message.reply_text(
                f"Your chat ID {chat_id} was not found in the registered list.\n𝐍𝐨. 𝐨𝐟 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝 𝐮𝐬𝐞𝐫𝐬 = {chat_ids.shape[0]}"
            )

    def get_latest_report_command(self, update: Update, context: CallbackContext):
    ...
    def register_commands(self) -> None:
        ...
        self.dispatcher.add_handler(
            CommandHandler("register_chat_id", self.register_chat_id_command)
        )
        self.dispatcher.add_handler(
            CommandHandler("unregister_chat_id", self.unregister_chat_id_command)
        )
        self.dispatcher.add_handler(
            CommandHandler("get_latest_report", self.get_latest_report_command)
        )
    ...
    def collect_chat_ids(self) -> set:
        ...
        try:
            ...
            )  # Use the service bot instance to get updates
            chat_ids = set()
            ...
            self.mk1.logging.logger.error(
                f"(TelegramAPI.collect_chat_ids) Failed to collect chat IDs: {e}"
            )
            return set()
    ...
    def escape_markdown(self, text):
        ...
        escaped_text = re.sub(
            r"([{}])".format(re.escape(markdown_chars)), r"\\\1", text
        )
    ...
    def run_daily_news_report(self) -> None:
        ...
        while True:
            ...
            if jobs and jobs[0].last_run:
                self.mk1.logging.logger.info(
                    f"(TelegramAPI.run_daily_news_report) Last run time: {jobs[0].last_run}"
                )
                break
            time.sleep(10)
